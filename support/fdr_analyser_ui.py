...
def draw_ap_graph(fdr):
    ...
    axes[i].plot(time, fdr['data.high_aoa_protection'], label='High-AoA', linewidth=5.0, color="orange")
    axes[i].plot(time, fdr['data.alpha_floor_condition'], label='A.FLOOR', linewidth=5.0, color="magenta")
    axes[i].grid(False)
    axes[i].set_ylim(0, 1)
    axes[i].legend()
    i += 1
